...
(function(w){
	...
	w[_namespace] = function(){
		...
		if (navtype() != '') {
			...
			$(".Eleditor-inputarea .textarea").on('click', function () {
				// console.info('xxx')
				// var height = $(this).offset().top;
				// console.info(height);
				var xuyang = document.querySelector('.Eleditor-method');
				console.info(document.querySelector('.Eleditor-method'))
				window.setTimeout(function () {
					if (navtype() == 'a') { //安卓手机
						xuyang.scrollIntoViewIfNeeded();
						// $('section').animate({
						// 	scrollTop: height
						// }, "slow");
					} else { 
		...
		if( _inArray('insertImage', _arg.toolbars) && typeof WebUploader != 'undefined' ){
			...
			_controllerUploader = WebUploader.create({
						    ...
						    fileSingleSizeLimit: _arg.upload.fileSizeLimit ? (_arg.upload.fileSizeLimit*1024*1024)*5 : undefined,
						    accept: {
			...
			_stylebarUploader = WebUploader.create({
						    ...
						    accept: {
						        title: 'Images', extensions: 'gif,jpg,jpeg,bmp,png,webp',
								// mimeTypes: 'image/gif,image/jpg,image/jpeg,image/bmp,image/png,image/webp'  //作者的
