...
public class RichText : MaskableGraphic, ILayoutElement, IPointerClickHandler, IBeginDragHandler, IEndDragHandler, IPointerDownHandler, IPointerUpHandler
{
    ...
    public RichTextDelegateUtil.StringDelegate onLinkClick;
    ...
    public static string cIconPngPathFormat = "Download/Art/Icon/{0}.png";//"Assets/Atlas/{0}.png";
#else
    public static string cIconPngPathFormat = "Assets/Download/Art/Icon/{0}.png";//"Assets/Atlas/{0}.png";
#endif

    [TextArea(3, 10)]
    [SerializeField, Tooltip("文本内容")]
    string m_Text = ""; //文本内容
    ...
    RichTextParser m_TxtParser = null;
    ...
    RichTextParser txtParser
    {
        get
        {
            if (m_TxtParser == null)
            {
                m_TxtParser = new RichTextParser();
            }
            return m_TxtParser;
        }
    }
    ...
    class LineData
    {
        ...
        public void OnFinish()
        {
            ...
            {
                if (charDatas[i].cType == CharType.UnderWire)
                { //下划线
                    lineOffsetY = lineBottom;
                }
                else if (charDatas[i].cType == CharType.DeleteWire)
                { //删除线
    ...
    bool CalcColorBegin(CalcCharData ccData)
    {
        if (m_ParamDict.ContainsKey(ccData.i))
        {
            ...
            if (ccData.blockLen == RichTextParser.FIXED_LEN_COLOR_BLOCK)
            {//文字变色:<color=00ff00ff>此后的文字全部为绿色，直到再次使用<color=ffff00ff>此后的文字变成黄色，以此类推;
            ...
            else if (ccData.blockLen == RichTextParser.FIXED_LEN_COLOR_MORPH_BLOCK)
            {//渐变，由颜色一变到颜色二;
                float r = Convert.ToInt16(strParam.Substring(0, 2), 16) / 255.0f; //R
                float g = Convert.ToInt16(strParam.Substring(2, 2), 16) / 255.0f; //G
                float b = Convert.ToInt16(strParam.Substring(4, 2), 16) / 255.0f; //B
    ...
    void CalcEmote(CalcCharData ccData, ref CharacterInfo ci)
    {
        ...
        {
            if (RichTextParser.GetEmoteType(eidx) == RichTextParser.RICHTEXT_ANIM_EMOTE_BASE)
            {
                //动态表情;
                ...
                if (m_AssetDataDict.ContainsKey(assetPath))
                {
                    ...
                    ccData.assetData.aType = RichTextParser.RICHTEXT_ANIM_EMOTE_BASE;
            ...
            else
            {
                ...
                if (m_AssetDataDict.ContainsKey(assetPath))
                {
                    ccData.assetData = new AssetData();
                    ccData.assetData.assetPath = assetPath;
                    ccData.assetData.aType = RichTextParser.RICHTEXT_EMOTE_BASE;
                    ccData.assetData.obj = m_AssetDataDict[assetPath].obj;
                    ccData.assetData.width = m_AssetDataDict[assetPath].width;
                    ccData.assetData.height = m_AssetDataDict[assetPath].height;
                    ccData.charW = ccData.assetData.width;
    ...
    void CalcPic(CalcCharData ccData, ref CharacterInfo ci)
    {
        if (m_ParamDict.ContainsKey(ccData.i) && m_AssetDataDict.ContainsKey(m_ParamDict[ccData.i]))
        {
            ...
            ccData.assetData = new AssetData();
            ccData.assetData.assetPath = assetPath;
            ccData.assetData.aType = RichTextParser.RICHTEXT_RES_PATH_BASE;
            ccData.assetData.obj = m_AssetDataDict[assetPath].obj;
            ccData.assetData.width = m_AssetDataDict[assetPath].width;
            ccData.assetData.height = m_AssetDataDict[assetPath].height;
            ccData.charW = ccData.assetData.width;
    ...
    void CalcCIcon(CalcCharData ccData, ref CharacterInfo ci)
    {
        ...
        if (m_ParamDict.ContainsKey(ccData.i))
        {
            ...
            if (m_AssetDataDict.ContainsKey(assetPath))
            {
                ccData.assetData = new AssetData();
                ccData.assetData.assetPath = assetPath;
                ccData.assetData.aType = RichTextParser.RICHTEXT_CICON_BASE;
                ccData.assetData.obj = m_AssetDataDict[assetPath].obj;
                ccData.assetData.width = m_AssetDataDict[assetPath].width;
                ccData.assetData.height = m_AssetDataDict[assetPath].height;
                ccData.charW = ccData.assetData.width;
    ...
    void UpdateLines()
    {
        ...
        {
            ccData.blockType = m_StrMask[ccData.i] / RichTextParser.RICHTEXT_MULL_BASE * RichTextParser.RICHTEXT_MULL_BASE;
            ccData.blockLen = m_StrMask[ccData.i] - ccData.blockType;

            if (ccData.blockType == RichTextParser.RICHTEXT_COLOR_BASE)
            {//字体颜色
            ...
            else if (ccData.blockType == RichTextParser.RICHTEXT_COLOR_END)
            {//颜色结束块;
                CalcColorEnd(ccData);
                continue;
            }
            else if (ccData.blockType == RichTextParser.RICHTEXT_UNDER_LINE_BASE)
            {//下划线起始块;
                CalcUnderLineBegin(ccData);
                continue;
            }
            else if (ccData.blockType == RichTextParser.RICHTEXT_UNDER_LINE_END)
            {//下划线结束块;
            ...
            else if (ccData.blockType == RichTextParser.RICHTEXT_DELETE_LINE_BASE)
            {//删除线起始块;
                CalcDelLineBegin(ccData);
                continue;
            }
            else if (ccData.blockType == RichTextParser.RICHTEXT_DELETE_LINE_END)
            {//删除线结束块;
                CalcDelLineEnd(ccData);
                continue;
            }
            else if (ccData.blockType == RichTextParser.RICHTEXT_LINK_BASE)
            {//超链接起始块;
            ...
            else if (ccData.blockType == RichTextParser.RICHTEXT_LINK_END)
            {//超链接结束块;
            ...
            else if (ccData.blockType == RichTextParser.RICHTEXT_BOLD_BASE)
            {//加粗起始块;
                CalcBoldBegin(ccData);
                continue;
            }
            else if (ccData.blockType == RichTextParser.RICHTEXT_BOLD_END)
            {//加粗结束块;
                CalcBoldEnd(ccData);
                continue;
            }
            else if (ccData.blockType == RichTextParser.RICHTEXT_ITALIC_BASE)
            {//斜体起始块;
                CalcItalicBegin(ccData);
                continue;
            }
            else if (ccData.blockType == RichTextParser.RICHTEXT_ITALIC_END)
            {//斜体结束块;
                CalcItalicEnd(ccData);
                continue;
            }
            else if (ccData.blockType == RichTextParser.RICHTEXT_SIZE_BASE)
            {//字体大小起始块;
            ...
            else if (ccData.blockType == RichTextParser.RICHTEXT_SIZE_END)
            {//字体大小结束块;
            ...
            if (ccData.blockType == RichTextParser.RICHTEXT_EMOTE_BASE)
            {//表情;
                CalcEmote(ccData, ref ci);
            }
            else if (ccData.blockType == RichTextParser.RICHTEXT_RES_PATH_BASE)
            {//图片
                CalcPic(ccData, ref ci);
            }
            else if (ccData.blockType == RichTextParser.RICHTEXT_CICON_BASE)
            {//定制图标
            ...
            if (CalcNewLine(ccData, strLen, ref totalWidth, ref totalHeight))
            {//满行，或有换行符;
                break;
            }
    ...
    void PreLoadAsset()
    {
        ...
        {
            AssetData ad = null;
            if (m_PreLoadAssetDic[assetKeyList[i]] == RichTextParser.RICHTEXT_ANIM_EMOTE_BASE)
            {
            ...
            else if (m_PreLoadAssetDic[assetKeyList[i]] == RichTextParser.RICHTEXT_EMOTE_BASE ||
              m_PreLoadAssetDic[assetKeyList[i]] == RichTextParser.RICHTEXT_RES_PATH_BASE ||
              m_PreLoadAssetDic[assetKeyList[i]] == RichTextParser.RICHTEXT_CICON_BASE)
            {
    ...
    void UpdateAssetGo()
    {
        ...
        for (int i = 0; i < m_Lines.Count; i++)
        {
            ...
            {
                if (m_Lines[i].assetDatas[j].aType == RichTextParser.RICHTEXT_ANIM_EMOTE_BASE)
                {
                    //动态表情;
                    go = GameObject.Instantiate(m_Lines[i].assetDatas[j].obj) as UnityEngine.GameObject;
                }
                else if (m_Lines[i].assetDatas[j].aType == RichTextParser.RICHTEXT_EMOTE_BASE ||
                  m_Lines[i].assetDatas[j].aType == RichTextParser.RICHTEXT_RES_PATH_BASE ||
                  m_Lines[i].assetDatas[j].aType == RichTextParser.RICHTEXT_CICON_BASE)
                {
                    //静态表情、图片、定制图标;
                    go = new GameObject();
                ...
                if (go != null)
                {
                    go.AddComponent<RichTextAssetIdentify>();
    ...
    void ClearAssetGo()
    {
        RichTextAssetIdentify[] oldAssetIdentifyCs = gameObject.GetComponentsInChildren<RichTextAssetIdentify>();
    ...
    {
        //2017-07-11 19:14:58 WongJJ 改

        UnityEngine.Object assetObj = null;
#if UNITY_EDITOR && !ASSETBUNDLE_MODEL
        assetObj = AssetDatabase.LoadAssetAtPath(assetPath, assetType);
#else
        string path = assetPath.ToLower().Replace("png", "assetbundle");
        int index = path.LastIndexOf("/", StringComparison.CurrentCultureIgnoreCase);
        string name = path.Substring(index + 1).Replace(".assetbundle", "");

        using (WongJJ.Game.Core.AssetBundleLoader loader = new WongJJ.Game.Core.AssetBundleLoader(path, false))
        {
            assetObj = loader.LoadAsset2<Sprite>(name);
        }
