...
class RichTextParser
{
    ...
    void ParseSingle(string str, string rex, int ibase)
    {
        ...
        foreach (Match mtch in Regex.Matches(str, rex))
        {
			...
			if(ibase == RichTextParser.RICHTEXT_EMOTE_BASE){//表情
				int eidx = -1;
				if(int.TryParse(arg, out eidx))
				{
					if (GetEmoteType (eidx) == RichTextParser.RICHTEXT_ANIM_EMOTE_BASE) {
						//动态表情;
						string assetPath = string.Format(RichText.animEmotePrefabPathFormat, arg);
						if(!_preLoadAsset.ContainsKey(assetPath)){
							_preLoadAsset.Add(assetPath, RichTextParser.RICHTEXT_ANIM_EMOTE_BASE);
						}
					} else {
						string assetPath = string.Format(RichText.emotePngPathFormat, arg);
						if(!_preLoadAsset.ContainsKey(assetPath)){
							_preLoadAsset.Add(assetPath, RichTextParser.RICHTEXT_EMOTE_BASE);
						}
					}
				}
			} else if(ibase == RichTextParser.RICHTEXT_RES_PATH_BASE){//图片
				string assetPath = arg;
				if(!_preLoadAsset.ContainsKey(assetPath)){
					_preLoadAsset.Add(assetPath, RichTextParser.RICHTEXT_RES_PATH_BASE);
				}
			} else if(ibase == RichTextParser.RICHTEXT_CICON_BASE){//定制图标
				string assetPath = string.Format(RichText.cIconPngPathFormat, arg);
				if(!_preLoadAsset.ContainsKey(assetPath)){
					_preLoadAsset.Add(assetPath, RichTextParser.RICHTEXT_CICON_BASE);
				}
			}
			_parms[mtch.Index] = arg;
			_logStr.Append("type=" + (ibase / RichTextParser.RICHTEXT_MULL_BASE) + ", str=" + mtch.Value + ", arg=" + _parms[mtch.Index] + ", idx= " + mtch.Index + ", len= " + mtch.Length + "\n");
        }
    }
