...
namespace WongJJ.Game.Core
{
    ...
    public class Dispatcher : MonoBehaviour
    {
        ...
        public static void InvokeAsync<T>(Action<T> action, T obj)
        {
            if (!_instanceExists)
            {
                Debug.LogError("No Dispatcher exists in the scene. Actions will not be invoked!");
            ...
            else
            {
                lock (_lockObject)
                {
                    _actions.Enqueue(() => action(obj));
                }
            }
        }

        public static void InvokeAsync<T1, T2>(Action<T1, T2> action, T1 obj1, T2 obj2)
        {
            if (!_instanceExists)
            {
                Debug.LogError("No Dispatcher exists in the scene. Actions will not be invoked!");
            ...
            else
            {
                lock (_lockObject)
                {
                    _actions.Enqueue(() => action(obj1, obj2));
                }
            }
        }

        public static void InvokeAsync<T1, T2, T3>(Action<T1, T2, T3> action, T1 obj1, T2 obj2, T3 obj3)
        {
            if (!_instanceExists)
            {
                Debug.LogError("No Dispatcher exists in the scene. Actions will not be invoked!");
            ...
            else
            {
                lock (_lockObject)
                {
                    _actions.Enqueue(() => action(obj1, obj2, obj3));
                }
            }
        }

        public static void InvokeAsync<T1, T2, T3, T4>(Action<T1, T2, T3, T4> action, T1 obj1, T2 obj2, T3 obj3, T4 obj4)
        {
            if (!_instanceExists)
            {
                Debug.LogError("No Dispatcher exists in the scene. Actions will not be invoked!");
            ...
            else
            {
                lock (_lockObject)
                {
                    _actions.Enqueue(() => action(obj1, obj2, obj3, obj4));
                }
            }
        }
